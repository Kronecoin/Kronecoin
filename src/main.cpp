...
uint256 hashGenesisBlock("0x4f8e54339cfaf0be1be13d446a626eb78aabe5d5ad7601b0b2f2a2e5911853c5");
...
const string strMessageMagic = "KroneCoin Signed Message:\n";
...
{
    // KroneCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // KroneCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "krone genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1499546913;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3170173;
	    ...
	    assert(block.hashMerkleRoot == uint256("0xeeecc3933916402cbf0ff4c95189cf408aad297338bdbbd996574036231186c4"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KroneCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KroneCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KroneCoinMiner(CWallet *pwallet)
{
    printf("KroneCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("KroneCoin-miner");
    ...
    try { loop {
        ...
        printf("Running KroneCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KroneCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KroneCoinMiner, pwallet));
}
