...
// Copyright (c) 2011-2012 KroneCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("KroneCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #KroneCoinTEST0\r");
            Send(hSocket, "WHO #KroneCoinTEST0\r");
        } else {
            // randomly join #KroneCoin00-#KroneCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #KroneCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #KroneCoin%02d\r", channel_number).c_str());
        }
